...
import salesApiStore from "../../apiRequest/salesApi/salesApiStore.js";
...
const SaleList = () => {
    const {saleList,totalSaleList,saleListApi} = salesApiStore();
    ...
    useEffect(() => {
        (async ()=>{
            setLoder("");
            await saleListApi(1,perPage,0);
            setLoder("d-none");
        })()
    }, []);
    const handlePageClick = async (even) => {
        setLoder("");
        await saleListApi(even.selected,perPage,0);
        setLoder("d-none");
    };
    const getPerPageValue = async (even) => {
        setPerPage(parseInt(even.target.value));
        setLoder("");
        await saleListApi(1,even.target.value,0);
        setLoder("d-none");
    };
    ...
    const deleteSalesData = async (id) => {
        ...
        if (deleteResp.isConfirmed){
            let resp = await salesDeleteApi(id);
            if (resp){
                                                        ...
                                                        saleList.map((item,i)=>{
                                                            return (
                                                                <tr key={i}>
                                                                    ...
                                                                    <td>
                                                                        <p className="text-xs text-start mt-3 ">{item["sales"][0]["name"]}</p>
                                                                    </td>
                                        ...
                                        <div className="col-12 mt-5">
                                            <nav aria-label="Page navigation example">
                                                <ReactPaginate
                                                    ...
                                                    pageCount={totalSaleList / perPage}
